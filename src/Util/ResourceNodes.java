...
public class ResourceNodes {
	...
	public static void generateHay() throws SQLException {
		...
		for (Location loc : hayLocations) {
			loc.getBlock().setType(Material.AIR);
		}
		Collections.shuffle(hayLocations);
		for (int x = 0; x < 8; x++) {
			hayLocations.get(x).getBlock().setType(Material.HAY_BLOCK);
		}
	}
	...
	public static void generateSweetGum() throws SQLException {
		...
		for (Location loc : sweetGumlocations) {
			loc.getBlock().setType(Material.AIR);
		}
		Collections.shuffle(sweetGumlocations);
		for (int x = 0; x < 8; x++) {
			sweetGumlocations.get(x).getBlock().setTypeIdAndData(175, (byte) 4, true);
		}
