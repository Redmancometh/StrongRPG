		...
		if (p.getItemInHand().hasItemMeta()) {
			if (p.getItemInHand().getItemMeta().getDisplayName().equals("String")) {
				...
				if (amount >= 30 && !(dg.checkCompleted(19, p))) {
					...
					if (amount != 30) {
						p.getItemInHand().setAmount(amount - 30);
					} else {
				...
				if (!dg.checkInProgress(19, p) && (!dg.checkCompleted(19, p))) {
					...
					if (dg.checkCompleted(19, p)) {
						p.sendMessage(ChatColor.DARK_GREEN + "Archer: " + ChatColor.GREEN + "Aim well friend!");
					}
				}
			}
		} else {
			if (!dg.checkInProgress(19, p) && (!dg.checkCompleted(19, p))) {
				...
				if (dg.checkInProgress(19, p) && !dg.checkCompleted(19, p)) {
					p.sendMessage(ChatColor.DARK_GREEN + "Archer: " + ChatColor.GREEN + "How is it going?");
				} else {
					if (dg.checkCompleted(19, p)) {
						p.sendMessage(ChatColor.DARK_GREEN + "Archer: " + ChatColor.GREEN + "Aim well friend!");
					}
