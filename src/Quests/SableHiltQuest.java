		...
		if(p.getItemInHand().hasItemMeta()){
			if(p.getItemInHand().getItemMeta().getDisplayName().equals("Slime ball")){
				...
				if(!dg.checkInProgress(18,p)&&(!dg.checkCompleted(18,p))){
					...
					if(dg.checkCompleted(18,p)){p.sendMessage(ChatColor.DARK_GREEN+"Sable-Hilt: "+ChatColor.GREEN+"Hi again, "+p.getName()+"!");}
			}
		}
		else
		{
			if(!dg.checkInProgress(18,p)&&(!dg.checkCompleted(18,p)))
			{
				...
				if(dg.checkInProgress(18,p)&&!dg.checkCompleted(18,p)){p.sendMessage(ChatColor.DARK_GREEN+"Sable-Hilt: "+ChatColor.GREEN+"I asked for 15... Go get some more please.");}
				else{if(dg.checkCompleted(18,p)){p.sendMessage(ChatColor.DARK_GREEN+"Sable-Hilt: "+ChatColor.GREEN+"Hi again, "+p.getName()+"!");}}
			}
		}
	}
}