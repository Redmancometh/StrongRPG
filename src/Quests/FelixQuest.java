...
public class FelixQuest{
	...
	public void felixsQuest(){
		...
		if(p.getItemInHand().hasItemMeta()){
			if(p.getItemInHand().getItemMeta().getDisplayName().equals("Black Ink")){
				...
				//if(dg.checkInProgress(14,p)&&!dg.checkCompleted(14,p)){p.sendMessage(ChatColor.DARK_GREEN+"Felix: "+ChatColor.GREEN+"I need 15 Black Ink.");}
					...
					if(dg.checkCompleted(14,p)){p.sendMessage(ChatColor.DARK_GREEN+"Felix: "+ChatColor.GREEN+"My book is almost done, thanks to you!");}
				}
			}
		}
		else
		{
			...
			{
				if(dg.checkInProgress(14,p)&&!dg.checkCompleted(14,p)){p.sendMessage(ChatColor.DARK_GREEN+"Felix: "+ChatColor.GREEN+"I need 15 Black Ink.");}
				else{if(dg.checkCompleted(14,p)){p.sendMessage(ChatColor.DARK_GREEN+"Felix: "+ChatColor.GREEN+"My book is almost done, thanks to you!");}}
			}
		}
	}
}