		...
		if(p.getItemInHand().hasItemMeta()){
			if(p.getItemInHand().getItemMeta().getDisplayName().equals("Chicken Egg")){
				...
				if(p.getItemInHand().getAmount()>=16&&!(dg.checkCompleted(15, p))){
					...
					if(amount!=16){p.getItemInHand().setAmount(amount-16);}
				...
				//if(dg.checkInProgress(15,p)&&!dg.checkCompleted(15,p)){p.sendMessage(ChatColor.DARK_GREEN+"Baker: "+ChatColor.GREEN+"Can't wait to make that cake!");}
					...
					if(dg.checkCompleted(15,p)){p.sendMessage(ChatColor.DARK_GREEN+"Baker: "+ChatColor.GREEN+"The cake was very good! Thanks again "+p.getName()+"!");}
				}
			}
		}
		else{
			if(!dg.checkInProgress(15,p)&&(!dg.checkCompleted(15,p))){
				...
				if(dg.checkInProgress(15,p)&&!dg.checkCompleted(15,p)){p.sendMessage(ChatColor.DARK_GREEN+"Baker: "+ChatColor.GREEN+"Can't wait to make that cake!");}
				else{if(dg.checkCompleted(15,p)){p.sendMessage(ChatColor.DARK_GREEN+"Baker: "+ChatColor.GREEN+"The cake was very good! Thanks again "+p.getName()+"!");}}
			}
		}
	}
}