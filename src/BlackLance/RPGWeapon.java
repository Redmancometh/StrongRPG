...
public class RPGWeapon
{
		...
		for (ItemStack item : i)
		{
			...
			ItemMeta imeta = item.getItemMeta();
				...
				if(imeta.getLore().size()<3)
				{
				    ...
				    try{DBUtil.setWeaponData(name, damages[0], damages[1], rp.getUID());}
    ...
    public static int[] getDamages(ItemStack weapon)
    {
	    ...
	    else{damages[0]=Integer.parseInt(damage.substring(9,11));}
	    ...
	    if(hcheck.length==2)
	    {
		if(hcheck[1].charAt(1)==','||hcheck[1].charAt(1)==']'){damages[1]=Integer.parseInt(hcheck[1].substring(0,1));}
		else{damages[1]=Integer.parseInt(hcheck[1].substring(0,2));}
	    }
	    if(hcheck.length==3)
	    {
		if(hcheck[1].charAt(2)==','||hcheck[1].charAt(2)==']'){damages[1]=Integer.parseInt(hcheck[1].substring(0,2));}
