	...
	public void knightsQuest() {
		...
		if (p.getItemInHand().hasItemMeta()) {
			...
			//Not Shrimp Meat
				...
				if (dg.checkInProgress(1, p) && !dg.checkCompleted(1, p)) {
					p.sendMessage(ChatColor.DARK_GREEN + "Knight: " + ChatColor.GREEN + "Please hurry!");
				} else {
					//Message for after the Quest is Completed, change the 1 to whatever your Quest ID is
					if (dg.checkCompleted(1, p)) {
						p.sendMessage(ChatColor.DARK_GREEN + "Knight: " + ChatColor.GREEN + "Talk to the mayor of the town on the other side of this tunnel for another quest! They urgently require aid adventurer!");
					}
				}
			}
		}
