		...
		if (m != Material.SPIDER_EYE) {
			if (!dg.checkInProgress(3, p) && (!dg.checkCompleted(3, p))) {
				...
				if (dg.checkCompleted(3, p)) {
					p.sendMessage(ChatColor.DARK_GREEN + "Witch:" + ChatColor.GREEN + " I may have more work for you later...");
				}
			}
		} else {
			if (!dg.checkCompleted(3, p)) {
				...
				if (amount >= 15) {
					...
					p.sendMessage(ChatColor.DARK_GREEN + "Witch:" + ChatColor.GREEN + " I told you I need 15! Get out of my sight until you get 15!");
				}
			} else {
