...
import com.strongholdcraft.blacklance.blm;
	...
	public double generateHit(Player phitter) {
		if (phitter.getItemInHand() != null && phitter.getItemInHand().hasItemMeta() && phitter.getItemInHand().getItemMeta().hasLore()) {
			...
			if (phitter.getItemInHand().getItemMeta().getLore().toString().contains("Damage")) {
				...
				if (damage.charAt(10) == '-') {
					lowdmg = Integer.parseInt(damage.substring(9, 10));
				} else {
					lowdmg = Integer.parseInt(damage.substring(9, 11));
				}
				...
				if (hcheck.length == 2) {
					if (hcheck[1].charAt(1) == ',' || hcheck[1].charAt(1) == ']') {
						...
						highdmg = Integer.parseInt(hcheck[1].substring(0, 2));
					}
				}
				if (hcheck.length == 3) {
					if (hcheck[1].charAt(2) == ',' || hcheck[1].charAt(2) == ']') {
						...
						highdmg = Integer.parseInt(hcheck[1].substring(0, 3));
					}
				}
