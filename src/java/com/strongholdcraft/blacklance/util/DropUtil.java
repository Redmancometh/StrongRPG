...
import com.strongholdcraft.blacklance.quests.QuestChecks;
...
public class DropUtil implements Listener {
	...
	public double dropDecider(int level, Player killer, Entity paul) {
		...
		if (doDrop < (.30)) {
			...
			if (dropWheel >= (.55) && dropWheel <= (.95)) {
				dropUncommon(level, killer, paul);
			}
			if (dropWheel > (.95) && dropWheel < (98)) {
				dropRare(level, killer, paul);
			}
			if (dropWheel >= (.98) && dropWheel <= (1)) {
	...
	public void hide(Player p, CraftItem ci) {
		EntityHider eh = new EntityHider(blacklance, EntityHider.Policy.BLACKLIST);
