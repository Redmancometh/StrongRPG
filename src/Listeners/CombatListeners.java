...
//This Event
	...
	public void mobHittingPlayer(EntityDamageByEntityEvent event) throws ClassNotFoundException, NoSuchMethodException, SecurityException
	{
		...
		{
			if(event.getDamager() instanceof Player&&!(CitizensAPI.getNPCRegistry().isNPC(event.getDamager()))){event.setCancelled(true);}
			else
			{
				...
				//Check if it's a mob
				if(event.getDamager() instanceof LivingEntity){paul = (LivingEntity) event.getDamager();}
				else
				{
					...
					if(paul==null){event.setCancelled(true);}
					//Calculate Damage
				}
				if(paul!=null)
				{
	...
	public void playerHittingMob(EntityDamageByEntityEvent event)
	{
		...
		if(!(event.getEntity() instanceof Player))
		{
				...
				else
				{
					...
					{
						Projectile pj = (Projectile)event.getDamager();
						//Damager is a projectile, find out if the shooter is a player
				...
				if(p.isInsideVehicle()){p.getVehicle().remove();}
				else
				{
					if(hit>0)
					{
